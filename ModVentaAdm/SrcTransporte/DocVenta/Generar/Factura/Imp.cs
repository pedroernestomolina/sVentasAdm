...
{
    public class Imp: ImpGenerar, IFactura 
    {
        public override string TipoDocumento_Get { get { return "FACTURA - FISCAL"; } }


        public Imp()
            :base()
        {
            _tipoDocIsFactura = true;
        }

        protected override void GuardarDoc()
        {
            if (!Helpers.Msg.Abandonar("VERIFICO LOS MONTO DE ISLR/IGTF ?")) 
            {
                return;
            }
        ...
        private void GuardarDocFactura()
        {
            try
            {
                ...
                //
                var _idSitemaDocumento = Sistema.Id_SistDocumento_Factura;
                ...
                if (s05.Entidad == "")
                {
                    throw new Exception("SERIE [ ID ] DOCUMENTO NO ENCONTRADA");
                }
        ...
        private void GuardarDocFacturaTipoHojaServ()
        {
            try
            {
                var _fechaEmision = Ficha.DatosDoc.FechaEmision_Get;
                ...
                var _notasObs = NotasObserv_Get;
                //
                var _docRef = Ficha.Items.GetItems.Where(w => w.Item.IsItemHojaServ).Select(s =>
                {
                    var nr = new OOB.Transporte.Documento.Agregar.FacturaFromHojaServ.FichaDocRef()
                    {
                        codigoDoc = s.Item.Get_ItemHojaServ.Ficha.docCodigo,
                        fechaDoc = s.Item.Get_ItemHojaServ.DocFecha,
                        idDoc = s.Item.Get_ItemHojaServ.DocId,
                        montoDivisaDoc = s.Item.Get_ItemHojaServ.Monto,
                        numDoc = s.Item.Get_ItemHojaServ.DocNumero,
                        tipoDoc = s.Item.Get_ItemHojaServ.DocTipo,
                    };
                    return nr;
                }).ToList();
                //
                ...
                var _serieId = s05.Entidad;
                //
                var _lstTurnos = new List<OOB.Transporte.Documento.Agregar.FacturaFromHojaServ.Turno>();
                var itemsDet = Ficha.Items.GetItems.Select(s =>
                {
                    var _idDocRef = "";
                    var _numDocRef = "";
                    ...
                    if (s.Item.IsItemHojaServ)
                    {
                        _idDocRef = s.Item.Get_ItemHojaServ.Ficha.docId;
                        _numDocRef = s.Item.Get_ItemHojaServ.Ficha.docNumero;
                        _codDocRef = s.Item.Get_ItemHojaServ.Ficha.docCodigo;
