...
namespace ModVentaAdm.SrcTransporte.DocVenta.NotaCreditoAdm.GenerarAdm.Vista
{
    public partial class Frm : Form
    {
        ...
        {
            _controlador.Doc.DocGenerar.setDocumentoNro(TB_DOCUMENTO_NRO.Text);
        }
        private void TB_FACTOR_CAMBIO_Leave(object sender, EventArgs e)
        {
            var _factor = decimal.Parse(TB_FACTOR_CAMBIO.Text);
            _controlador.Doc.DocGenerar.setFactorCambio(_factor);
        }
        ...
        {
            _controlador.Doc.BuscarCliente();
            TB_CADENA.Text = _controlador.Doc.Get_CadenaBusq;
            actualizarData();
            actualizarTotales();
            actualizarDatosDoc();
        ...
        {
            L_CLIENTE.Text = _controlador.Doc.Get_EntidadAplicarNotaCredito_DatosCliente;
            DTP_FECHA_EMISION.Value = _controlador.Doc.DocGenerar.Get_FechaEmision;
            TB_DOCUMENTO_NRO.Text = _controlador.Doc.DocGenerar.Get_DocNumero;
            TB_FACTOR_CAMBIO.Text = _controlador.Doc.DocGenerar.Get_TasaCambio.ToString("n2").Replace(".", "");
        }
        void actualizarData() 
        {
