...
namespace ModVentaAdm.Src.CxC.Tools.GestionPagoZufu.handler.Ctas
{
    public class listaCtasPend
    {
        ...
        public decimal GetTotalMontoPend { get { return _bl.Sum(s => s.montoResta * s.SignoDoc); } }
        ...
        public void setData(List<itemCtaPend> lst)
        {
            _bl.Clear();
        ...
        public void ItemActualSetMontoAbonar(decimal monto)
        {
            if (ItemActual == null) return;
            ((itemCtaPend)ItemActual).setMontoAbonar(monto);
            _bs.CurrencyManager.Refresh();
        }
        public void ItemActualSetDetalleAbono(string p)
        {
            if (ItemActual == null) return;
